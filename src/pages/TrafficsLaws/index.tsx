...
const TrafficsLawsPage = (): ReactElement => {
    ...
    const isModalOpened: boolean = useSelector(
        (state: TRootState) => state.application.isModalOpened,
    );
    const modalContentName: TContentItem = useSelector(
    ...
    const title = hasModalContent
        ? (modalContentName as IContentLabel).title || ""
        : "";
    ...
    const PoliceManModalContent = (): ReactElement => {
        ...
        return (
            <div className="tl-modal-content">
                {content.additionalInfo?.pointsTextList.map(
                    (item: string, index: number) => (
                        <DefaultImageCard
                            key={index}
                            imageId={
                                modalPolicemanImagesPaths[content.title!][index]
                            }
                            title={
                                splitString(
                                    content.additionalInfo?.pointsTextList[
                                        index
                                    ] as string,
                                )[0]
                            }
                ...
                {content.additionalInfo?.pointsTextList.map(
                    (_item: string, index: number) => (
                        <DefaultImageCard
                            ...
                            title={
                                content.additionalInfo?.pointsTextList[index]
                            }
                        />
                    ),
                )}
            ...
            <div className="traffics-laws-page">
                ...
                >
                    {shouldOpenModal && hasModalContent && (
                        <div>
                            <p className="tl-modal-title">
                                {(modalContentName as IContentLabel).title}
                            </p>
                            <div className="tl-modal-content">
                                {title in modalPolicemanImagesPaths && (
                                    <PoliceManModalContent />
                                )}
                                {title in signsImagesPaths && (
                                    <SignModalContent />
                                )}
                            </div>
                        </div>
